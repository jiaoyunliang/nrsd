      ...
      this.$refs["dynamicValidateForm"].validate(valid => {
        if (valid) {
          ...
          if (confirm("确认提交?")) {
            ...
            console.log(JSON.stringify(_this.currentData));
            _this.$http
              .post(
                getCtx() + "/enterprise/qua/insertproducequadetailinfo",
                this.currentData,
                {
                  Accept: "application/json",
                  "Content-Type": "application/json"
              ...
              .catch(err => {
                _this.loading = false;
                console.log("api handleSaveProductQua error", err);
              });
          }
        } else {
          return false;
  ...
  methods: {
    ...
    doPack: function() {
      ...
      if (confirm("确认提交?")) {
        _this.$http
          .post(
            getCtx() + "/enterprise/qua/productDoPackage",
            _this.packDataState,
            {
              Accept: "application/json",
              "Content-Type": "application/json"
          ...
          .catch(err => {
            _this.loading = false;
            console.log("api productDoPackage error", err);
          });
      }
    },
    fetchData: function() {
      ...
      let _this = this;
      _this.$http
        .post(getCtx() + "/enterprise/qua/productPackageListState", {})
        .then(rs => {
          if (rs.data.result === 200) {
        ...
        .catch(err => {
          console.log("productPackageListState error", err);
        });
    }
  }
});
...
Vue.component("qua-packagerecord-component", {
  ...
  methods: {
    ...
    fetchData() {
      ...
      _this.loading = true;
      this.$http
        .post(getCtx() + "/enterprise/qua/productPackageList", params, {
          Accept: "application/json",
          "Content-Type": "application/json"
        })
        .then(rs => {
        ...
        .catch(err => {
          _this.loading = false;
          console.log("api productPackageList error", err);
        });
    }
  }
});
...
const maintab = new Vue({
  ...
  methods: {
    ...
    fetchData() {
      ...
      this.$http
        .post(getCtx() + "/enterprise/qua/productquainfo", params, {
          Accept: "application/json",
          "Content-Type": "application/json"
        })
        .then(rs => {
        ...
        .catch(err => {
          _this.loading = false;
          console.log("api productquainfo error", err);
        });
    },
    ...
    handleSearchDetail: function(row) {
      location.href =
        getCtx() +
        "/enterprise/qua/productquadetail?quaCode=" +
        row.quaCode +
        "&quaName=" +
        row.quaName;
    },
    handleAddProductQua: function() {
      let _this = this;
