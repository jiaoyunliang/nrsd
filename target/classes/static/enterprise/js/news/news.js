...
new Vue({
    ...
    methods: {
        ...
        fetchData() {
            ...
            axios
                .post(getCtx()+"/enterprise/news/queryNewsPage", params)
                .then(rs => {
                    if(rs.status == 200){
        ...
        newsHref:function (data) {
            if(data.isRed==1){
                return "<a style='color: #f56c6c' href='"+getCtx()+"/enterprise/news/detail?id="+data.id+"'>"+data.title+"</a>"
            } else {
                return "<a href='"+getCtx()+"/enterprise/news/detail?id="+data.id+"'>"+data.title+"</a>"
            }
        }
    }
});
